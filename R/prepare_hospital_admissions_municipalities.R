#' Prepare a tibble with hospital admissions count for municipalities
#
#' @param hospital_admissions_data Data from `fetch_hospital_admissions_data` function.
#' @param uf_code UF code. Defaults to 22.
#' @return A tibble.
#' @importFrom rlang .data
prepare_hospital_admissions_municipalities <- function(hospital_admissions_data, uf_code = 22){
  ...
  res1 <- hospital_admissions_data %>%
    ...
    dplyr::group_by(date = .data$DT_INTER, cod_mun = .data$MUNIC_RES) %>%
  ...
  # Full join
  res <- dplyr::full_join(x = res1, y = res2, by = c("date", "cod_mun")) %>%
